...
#	endif
	...
	strncat_s(logFilePath, Time::s_programStartTime.getStr(), std::strlen(Time::s_programStartTime.getStr()));
	strcat_s(logFilePath, "_");
	strcat_s(logFilePath, LOG_DEBUG_CHAR);
	strcat_s(logFilePath, DIR_DELIMITER);
	strcat_s(logFilePath, "EU__");
	strncat_s(logFilePath, Time::s_programStartTime.getStr(), std::strlen(Time::s_programStartTime.getStr()));
	strcat_s(logFilePath, "_");
	strcat_s(logFilePath, LOG_DEBUG_CHAR);
...
void Log::startup() {
	...
	strncat_s(commandBuffer, Time::s_programStartTime.getStr(), std::strlen(Time::s_programStartTime.getStr()));
	strcat_s(commandBuffer, "_");
	strcat_s(commandBuffer, LOG_DEBUG_CHAR);
...
#endif
	std::cout << std::setw(36) << "\x1B[35m[START DATETIME]\033[0m" << Time::s_datetime.getNowStr() << std::endl;
...
#endif
		logFile << std::setw(36) << "[START DATETIME]" << Time::s_datetime.getNowStr() << std::endl;
		logFile << line << std::endl;
	} else {

		std::cout << std::left 
			<< "\x1B[31m[" << Time::s_time.getNowStr() << "]  " << "(ERROR)\n"
